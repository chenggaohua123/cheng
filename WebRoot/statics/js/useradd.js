...
var errorinfo=null;
var work_idPicPath=null;
var errorinfo_work=null;
...
$(function(){
	...
	if(errorinfo.val()==null || errorinfo.val()==""){
		a_idPicPath.next().html("* 上传文件不能超过500kb,*上传文件类型必须是:jpg、jpeg、png、pneg");
	}else{
		a_idPicPath.next().html(errorinfo.val());
	}
	if(errorinfo_work.val()==null || errorinfo_work.val()==""){
		a_workPicPath.next().html("* 上传文件不能超过500kb,上传文件类型必须是:jpg、jpeg、png、pneg");
	}else{
		a_workPicPath.next().html(errorinfo_work.val());
	}
	$.ajax({
		type:"GET",//请求类型
		url:path+"/role/getallroles",//请求的url
		data:null,//请求参数
	...
	userCode.bind("blur",function(){
		...
		$.ajax({
			...
			data: {userCode:userCode.val()},/*{method:"ucexist",userCode:userCode.val()} "userCode="+userCode.val()*///请求参数
			dataType:"json",//ajax接口（请求url）返回的数据类型
			success:function(data){//data：返回数据（json对象）
				if(data.userCode == "exist"){//账号已存在，错误提示
					validateTip(userCode.next(),{"color":"red"},imgNo+ " 该用户账号已存在",false);
				}else if(data.userCode == "Empty"){
