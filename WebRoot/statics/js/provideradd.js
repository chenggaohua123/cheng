...
$(function(){
	...
	if(errorinfo.val()==null || errorinfo.val()==""){
		companyLicPicPath.next().html("* 上传文件不能超过500kb,*上传文件类型必须是:jpg、jpeg、png、pneg");
	}else{
		companyLicPicPath.next().html(errorinfo.val());
	}
	
	if(errorinfo_work.val()==null || errorinfo_work.val()==""){
		orgCodePicPath.next().html("* 上传文件不能超过500kb,*上传文件类型必须是:jpg、jpeg、png、pneg");
	}else{
		orgCodePicPath.next().html(errorinfo_work.val());
	}
	/*
	 * 验证
	 * 失焦\获焦
